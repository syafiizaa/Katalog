...
Matching is exact on normalized name+variant (lowercase, collapsed spaces). On top of that, a **fuzzy "mungkin sama" detector** (`levenshtein`/`strSim`/`wordPrefix`/`bestMatch`, threshold `FUZZY_THRESHOLD`) flags items whose name is only a spelling/spacing/typo/base-name variation of an entry on the other side (e.g. "Exlusive" vs "Exclusive", "Rapika" vs "Rapika Botol"). These are **labelled, not auto-merged** — each row is tagged new-product / new-variant / mungkin-sama, with a "hide likely duplicates" toggle to focus on truly-new items.
