...
- **Modals**: a 2-step add-to-cart flow — variant modal (pick variant) → add-cart modal (quantity + unit + free-text notes). Closed via overlay click or Escape.
- **Units**: `UNITS` (top of file, next to `CONFIG`) lists the sale units offered in the add-cart modal — currently `Pcs` / `Pak` / `Lusin`, default `Pcs`. The buttons are generated by `renderUnitOptions()`, so adding a unit means editing that array only. `normalizeUnit()` maps anything unknown/missing back to `Pcs`, which is also how carts saved before this feature existed get migrated on load.
- **Cart**: persisted to `localStorage`; line items are keyed by `nama + variant + unit + notes`, so the same product with a different unit (or different notes) is a separate line — 2 pak and 2 pcs must never be summed.
- **WhatsApp checkout**: `checkoutWhatsApp()` builds a text order (items, qty + unit, notes, pickup time) and opens a `https://wa.me/<number>?text=...` link.
...
**Excel export:** the "Ekspor Excel" button in the header calls `exportExcel()` → `buildExportSheets()`, writing one `.xlsx` workbook. Before a `.sql` is loaded it contains **Ringkasan** + **Katalog Lengkap**; after comparing it adds **Perlu Ditambah**, **Hanya di Katalog**, and **Data Kasir**. Every sheet is one row per (produk, varian) unit, sorted A→Z with `byText()` (locale-aware + `numeric: true`, so "50ml" sorts before "100ml"), with a frozen bold header row and an autofilter.

The workbook is written by **`xlsx.js`** — a dependency-free `.xlsx` writer (~330 lines) that assembles the OOXML parts and the ZIP container (CRC32 + stored/uncompressed entries) by hand, so the site stays static with no CDN or npm. Public API: `downloadXlsx(filename, sheets)` / `buildXlsx(sheets)`, where a sheet is `{ name, cols, header, rows }`; cell values are string → text, number → numeric, and `{ v, s: XLSX_STYLE.* }` picks a style. **`XLSX_STYLE` indices must stay in the same order as the `<xf>` entries in `<cellXfs>`** inside `XLSX_STYLES`. `xlsx.js` must be uploaded to Netlify along with `compare.html`.
...
`gambar` accepts either a full external URL or a repo-relative path into `img/` (e.g. `img/alat tulis.jpg`) — both forms are in use today, so `img/` must be deployed too. Nothing links the two automatically: a file dropped into `img/` is only used once some product's `gambar` points at it.
